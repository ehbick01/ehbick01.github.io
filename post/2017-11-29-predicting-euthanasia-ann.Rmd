...
# Read in LMAS data
lmas_data_raw <- read_csv("https://data.louisvilleky.gov/sites/default/files/Animal_IO_Data_1.csv")
...
estimates_euth_tbl %>% conf_mat(truth, estimate)

```

The way to read the confusion matrix is:
+ For every animal predicted to be euthanized in our test set, 13,879 animals were in fact euthanized (these are **true positives**)
+ For every animal predicted to be euthanized, 42 actually were not (these are **false positives**)
+ For every animal not predicted to be euthanized, 15,127 animals were in fact not euthanized (these are **true negatives**)
+ For every animal not predicted ot be euthanized, 4 actually were euthanized (these are **false negatives**)
...
<!-- LIME stands for *Local Interpretable Model-agnostic Explanations* - and through feature importance is a valuable tool for understanding which features tend to lend the most influence on a given machine learning model (such as our own ANN). The process performed to do this varies by model type, but in general feature importance looks for variables that tend to influence some loss or accuracy function most significantly.  -->

<!-- To perform this process on our own model, we need to identify the model type for our model and run predictions on new data to return feature importance. -->

<!-- ```{r lime-setup, echo = TRUE, include = TRUE} -->

<!-- # Identify class -->
<!-- class(keras_init) -->

<!-- # Setup lime::model_type() function for our ANN classification model -->
<!-- model_type.keras.models.Sequential <- function(x, ...) { -->

<!--     return("classification") -->
<!-- } -->

<!-- # Setup lime::predict_model() function for our ANN model -->
...
<!-- predict_model(x = keras_init,  -->
<!--               newdata = x_euth_test_tbl,  -->
<!--               type = 'raw') %>% -->
<!--     tibble::as_tibble() -->

<!-- ``` -->

<!-- Once the helper functions are in place for us to apply to our ANN model, we can then build out the variable explaination using the `lime()` function and then using the `explain()` function we can return the top features for every observation (if we wanted - I'm going to run for just the first 10). -->

<!-- ```{r run-lime, echo = TRUE, include = TRUE} -->
...
<!-- Once the explanation model is finalized, we can plot the top four input variables from an importance perspective. The colors of the bars are important - green bars are supporting the model conclusion for the observation (they support a "yes" prediction when the value truth is "yes"), and red bars contradict (they support a "no" prediction when the value truth is "yes"). -->

<!-- ```{r plot-importance, echo = TRUE, include = TRUE} -->
...
<!-- As you can see, for the first ten cases - the logged age values is an extremely important predictor of euthanasia. This seems about right, as age seems like it would be a strong determinant for euthanizing (either due to litters and limited space, or due to health issues at old age). -->
...
corr_plot <- corrr_analysis %>%
  ...
  geom_segment(aes(xend = 0, yend = feature), 
    ...
    labs(title = "Animal Euthanasia Correlation Analysis",
         subtitle = "Positive Correlations (contribute to euthanasia), Negative Correlations (prevent euthanasia)",
         y = "Feature",
         x = "Correlation") + 
